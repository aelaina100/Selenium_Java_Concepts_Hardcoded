   ...
   public void A_demonstrational() throws InterruptedException
	{
	    ...
	    System.setProperty("webdriver.chrome.driver", "C:\\Program Files\\Third_Party_Browsers\\chromedriver-win64\\chromedriver.exe"); //to utilize Selenium Manager, ignore it
		ChromeOptions options = new ChromeOptions();      //to utilize Selenium Manager, ignore it
		options.addArguments("--remote-allow-origins=*");  //to utilize Selenium Manager, ignore it
		WebDriver driver = new ChromeDriver(options);  //to utilize Selenium Manager, only use this line !
		
		// Selenium Manager is introduced automatically in Selenium jars as of Selenium 4.6.0 (Check your Selenium dependencies):=====>
		// Where you can only have, instead of the 4 lines above:   WebDriver driver= new ChromeDriver();  Try it !
		// Where Selenium Manageris going to find the compatible third party browser extension, download it, & set it as in system property.
	   ...
	   WebDriver driver = new EdgeDriver(options);  */
		...
		driver.get("https://ca.yahoo.com/"); // Implicit waits are BAD- b4 the Yahoo page even begin loading, the below line executes. Hence giving the wrong page title
		                             // This happens sometimes. So ALWAYS use Explicit wait that points to an element unique on the yahoo page (has 0 to do with google page)
