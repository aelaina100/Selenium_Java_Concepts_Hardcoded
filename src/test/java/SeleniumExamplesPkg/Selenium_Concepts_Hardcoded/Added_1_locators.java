...
//is not clickable at point (274, 354). Other element would receive the click: 
   ...
   // Second TestCase (IMPORTANT):
	...
	public void C_SalesForceTest()
	{
		...
		// But keep in mind that it's too long===> you can have:        
		 *                                                           input.input                (As long as it UNIQUELY identifies the correct intended element)  OR
		      line A                                                 input.input.r4             (As long as it UNIQUELY identifies the correct intended element)
		      line B                                                 input.input.r4.wide                    // // // // // // // // // // // // // // // // // //
		      line C                                                 input.input.r4.wide.mb16               // // // // // // // // // // // // // // // // // //
		      line D                                                 input.input.r4.wide.mb16.mt8           // // // // // // // // // // // // // // // // // //
		      line E                                                 input.input.r4.wide.mb16.mt8.username  // // // // // // // // // // // // // // // // // //
		                                                                
		/
		Or simply use ClassName locator (Takes a priority the above CSS locator  TagName.class  or .class) 
		where,in here, 6 seperate values are given(and seperated by spaces) as long as it UNIQUELY identifies the intended element. NEVRETHELESS,
		Sometimes the compiler (And not always), in the case of compound classes, will imply that it ONLY accepts a CSS locator...1000!
		
		CRUCIAL: For any locator on lines B to E ===> the locations of the class values can be changed and/or class value(s) can be taken out as long as
		         the final locator UNIQUELY identifies the intended element. For example:============>
		         
		    input.input.r4.wide.mb16.mt8.username  =   input.username.input.r4.wide.mb16.mt8  =   input.wide.mt8.username.mb16.input= 
		    input.input.r4.wide.username   =   input.input.username =input.username  = ...many more.... = All point out to the Username field (Exact same element).
		*/
		
		// or even the below line:
		//driver.findElement(By.xpath("//input[@class= 'input r4 wide mb16 mt8 username']")).sendKeys("USERNAMMMMAEE");
		 // In the above xpath, the full value MUST  be given ''input r4 wide mb16 mt8 username'..100 ! & you can not re-arrange the string between the spaces.
...
/* Note ALPHA : Assume you have the following HTML code associated with a Username field =========>
 * <input class="form-control ng-pristine ng-invalid ng-touched"   minlength="2"   name="name"   required=""  type="text">
 * 
 * Where the values of the attributes 'name' & 'type' are not unique. Then in terms of priority follow the below steps sequentially:
 * 
 * #1- Validate whether any of the class values uniquely identifies the Username field.
 * 
 *    If yes===> then use the className locator.  [[Nevertheless Sometimes the compiler (Not always), in the case of compound classes,implies it ONLY accepts 
 *    a CSS locator...1000! so move on to the #2 technique]]
 *    
 *    If No====> move on to the #2 technique
 *    
 *    
 * #2- If not, then validate the CSS Selector of:     input.form-control.ng-pristine.ng-invalid.ng-touched"  to be unique
 *       If it is not, then ========> move on to a different locator technique.
 *       
 *       If it is, then ============> Check if you can shorten it out to the maximum possible.
 *    
