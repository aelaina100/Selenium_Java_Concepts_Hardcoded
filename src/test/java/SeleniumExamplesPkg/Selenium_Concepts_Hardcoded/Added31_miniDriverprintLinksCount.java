...
/* TC: 
 	// On: https://www.rahulshettyacademy.com/AutomationPractice/
		...
		// 7- In the footerSection section, count the number of Links sharing a bigger font.
...
public class Added31_miniDriverprintLinksCount {
	...
	public void printLinksCountTest()
	{
     ...
     //3: Done by limiting the scope of the web driver (driver) to the 1st column only of the footer:
      wait.until(ExpectedConditions.visibilityOf(footer.findElement(By.xpath("(//td/ul)[1]")))); 
      ...
      //Wait logic: Once the box element (footer section) becomes 'visible',then the links inside it become so at the same time too.
      WebElement firstColumnInFooter= footer.findElement(By.xpath("(//td/ul)[1]"));
      ...
      wait.until(ExpectedConditions.visibilityOf(footer.findElement(By.xpath("(//td/ul)[2]"))));
      // Using .visibilityOf(WebElement element) instead of visibilityOfElementLocated(By Locator) which can also be used instead in here.
      //Wait logic: Once the box element (footer section) becomes 'visible',then the links inside it become so at the same time too.
      WebElement secondColumnInFooter= footer.findElement(By.xpath("(//td/ul)[2]"));
      ...
      WebElement thirdColumnInFooter= footer.findElement(By.xpath("(//td/ul)[3]"));
      ...
      wait.until(ExpectedConditions.visibilityOf(footer.findElement(By.xpath("(//td/ul)[4]"))));
      WebElement fourthColumnInFooter= footer.findElement(By.xpath("(//td/ul)[4]"));
      ...
      System.out.println(footer.findElements(By.cssSelector("h3 a")).size()); 
     ...
     for(int i=0; i< linksInFooter.size(); i++){
         ...
         a.moveToElement(linksInFooter.get(i)).keyDown(Keys.SHIFT).click().build().perform();
     }
